...
// Initial seed data (used only for first-time database initialization)
...
export async function seedDatabase() {
    try {
        ...
        await db.transaction('rw', db.properties, db.tenants, db.documents, db.issues, db.costs, async () => {
            ...
            console.log(`✅ Migrated ${INITIAL_PROPERTIES.length} properties`);

            // Add tenants
            await db.tenants.bulkAdd(INITIAL_TENANTS);
            console.log(`✅ Migrated ${INITIAL_TENANTS.length} tenants`);

            // Add documents
            await db.documents.bulkAdd(INITIAL_DOCUMENTS);
            console.log(`✅ Migrated ${INITIAL_DOCUMENTS.length} documents`);

            // Add issues
            await db.issues.bulkAdd(INITIAL_ISSUES);
            console.log(`✅ Migrated ${INITIAL_ISSUES.length} issues`);

            // Add costs
            await db.costs.bulkAdd(INITIAL_COSTS);
            console.log(`✅ Migrated ${INITIAL_COSTS.length} costs`);
        });
